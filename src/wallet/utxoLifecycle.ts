 ...
 * (and `spentBy` when the spender is known), never by deleting it. Unknown
 * spender → quarantine diagnostic, not a fake empty `spentBy`. Restore /
...
const LEGACY_STATUS: Record<
  string,
  { spendable: boolean; spentBy: string | null; reserved: boolean }
> = {
...
/** Hidden without a spender (`spendable: false`, no `spentBy`) — thawable unless quarantined. */
export function isUnspendable(rec: UtxoLockRecord): boolean {
  return !rec.spendable && rec.spentBy == null && rec.lockOwnerId == null;
}

/** Chain-spent, spender body not inserted. Not a consume; not a 24h thaw. */
export function isQuarantined(rec: UtxoLockRecord): boolean {
  return Boolean(rec.diagnostic?.startsWith("quarantine:"));
}

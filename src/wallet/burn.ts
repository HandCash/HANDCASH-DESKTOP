...
async function signBurnInputs(args: {
  ...
  const byOutpoint = new Map(args.inputs.map((input) => [wireOutpoint(input.outpoint), input]))
  ...
  for (const vin of vins) {
    ...
    if (!source) throw new Error(`Burn input ${outpoint} is missing its source output`)
  ...
  const feeSatoshis = sourceValues.every(
  ...
  const result = await executeBurnLifecycle(args.plan, {
    build: async () => {
      const created = await args.active.wallet.createAction({
        description:
          args.plan.path === 'burnBsv21' ? 'Burn BSV-21 token' : 'Burn collectables',
      ...
      if (!signable) throw new Error('Burn did not return a signable transaction')
    ...
    refresh: () =>
      refreshFromChainDuringSpend({ forceReview: true, announceReceive: false }).then(
        () => undefined,
      ),
  ...
  return Promise.all(
    tips.map(async (tip) => {
      ...
      try {
        ...
        const lockingScript = beef.findTxid(txid)?.tx?.outputs[vout]?.lockingScript?.toHex()
...
export async function burnBsv21(args: {
  ...
  return runExclusiveSpend(async () => {
    ...
    const pendingId = `burn-${Date.now()}-${Math.random().toString(16).slice(2, 8)}`
    const item = {
      name: token.sym,
    ...
    if (plan.path === 'refuse') {
      ...
      throw refusalMessage(plan.reason)
    }
    try {
...
export async function burnOneSat(
  ...
  return runExclusiveSpend(async () => {
    ...
    const item = {
      ...
      origin:
        firstHeld?.origin ??
        wireOutpoint(outpoints[0] ?? '').replace('.', '_'),
    ...
    if (plan.path === 'refuse') {
      upsertAppActivity({
        ...
        sats: outpoints.length,
        method: 'burn-collectable',
        note: `${item.name} were not burned`,
        item,
        burn: { asset: '1sat', destroyedAmount: String(outpoints.length) },
        status: 'failed',
        pendingId,

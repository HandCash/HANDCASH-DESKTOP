...
Basket membership and tags are **local wallet state** and never reach the receiver. A latch delivered as a bare 1-satoshi P2PKH output is therefore byte-identical to a tip: a receiver scanning for 1-sat outputs imports it as a collectable, and origin resolvers that walk prior inputs bind it to the tip's origin, producing a **duplicate item**.

Accordingly, a latch output MUST be distinguishable from a tip by inspection of the transaction alone:

* Its locking script MUST carry a latch marker (e.g. a pushdata prefix before the ownership branch), **and**
* Its value MUST NOT be exactly 1 satoshi.

Receivers MUST route marked latch outputs to `1sat-latch` and MUST NOT list them as collectables. Receivers SHOULD deduplicate basket `1sat` by origin, keeping the best-attested tip, since an origin has exactly one live tip.
...
### Phase 1a — Marked latch sends ⏳

Soft-latch sends with a **bare P2PKH latch** were implemented and withdrawn: the unmarked 1-sat latch imported as a duplicate collectable on receiving wallets (see *Latch outputs MUST be identifiable on chain*). `isLatchedSendEnabled()` returns `false` until the latch carries a marker script and a non-1-satoshi value; sends fall back to [BRC-150](./0150.md) v2 remittance.
...
* **HandCash Desktop** (reference, v3 verify live; latched send pending marker script): `oneSatLatch.ts`, `oneSatProvenance.verifyProvenance`, `collectables.sendCollectable`.  

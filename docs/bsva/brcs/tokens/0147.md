...
| `collection:<id>` | MAY | Collection filter key. |
| `creator:<id>` | MAY | Creator filter key. Distinct from `app:`. |
| `id:<key>` | SHOULD for held rows | BRC-164 local list/spend key. Not asset identity or provenance. |
...
* **Fungible misfile** — Auto-importing every bare 1-sat that spends a 1-sat parent will file [BRC-175](./0175.md) transfer tips into `1sat`. Those tips then look burnable as junk NFTs. See Compatibility above.

...
Capability-aware identity-key peers MAY use hardened BRC-156 when the wallet
send bridge is enabled.
...
* **HandCash Desktop** (reference — soft-latch live send; BRC-150 v2 authenticity;
  alternating delayed-proof covenant + bounded verifier under test; hardened
  wallet send bridge gated off until createAction unlock lands):
  `oneSatLatch.ts`, `oneSatHardenedLatch.ts`, `oneSatAuthenticity.ts`,
  `oneSatImport.resolveLatchedTip`, `collectables.sendCollectable`.  

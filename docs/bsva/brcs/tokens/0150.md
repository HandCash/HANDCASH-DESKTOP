...
5. **Tip binding (parent remittance — pre-broadcast)** — If step 4 fails, receivers **SHOULD** accept the package when all of the following hold (a pre-broadcast remittance that inherits origin onto held tip `T`):
...
**Parent inherit:** a pre-broadcast remittance may prove parent `P`, then the
receiver holds new tip `T`. Checking only “`T` spends `P`” accepts the bad `o0`
above. Step 5 therefore also requires sat ordering of that vin onto `T`’s vout
(preceding sats known).
...
**Parent remittance:** when the remittance still names spent tip `P` and the
receiver holds new tip `T`, include (or link) the preceding input sources for
`T` with the package or with `T` so inherit does not force an extra network hunt
to run sat ordering on `T → P`.

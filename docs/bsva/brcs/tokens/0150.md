...
Out of scope: OrdLock marketplace contracts, BSV-20/21, minting APIs, redefinition of 1Sat origin theory, and basket naming/tags (see [BRC-147](./0147.md)).
...
| `beefB64` | string | MUST | Base64 encoding of **BEEF** ([BRC-62](../transactions/0062.md)) covering every `path` transaction **and** every preceding input source required for FIFO sat assignment on each hop. AtomicBEEF ([BRC-95](../transactions/0095.md)) is allowed only when it still covers that set. |
...
2. **Prefer extending a prior remittance** when the tip (or its immediate parent tip) already carries a verified v2 package: prepend the tip being proven to `path` and merge that tip’s transaction **plus preceding input sources needed for FIFO** into `beefB64`. Do **not** re-walk full tip→origin lineage when an inductive extension verifies.
3. Otherwise construct `path` tip→origin such that each consecutive pair with different transaction ids is related by **1Sat FIFO sat assignment**: the parent 1-sat outpoint is the child input `vin` whose satoshis land on the child `vout` under
   `sum(input sats before vin) === sum(output sats before vout)`, and that input is exactly 1 satoshi. A mere spend of the parent as *some* input is not sufficient — funding inputs that precede the ordinal `vin` can assign a different sat to the claimed vout.
4. Assemble `beefB64` covering every transaction id appearing in `path`, **and** the source transaction of every input that precedes (and including) the ordinal-mapping `vin` on each hop. Canonical transfers (`i0` 1-sat → `o0` 1-sat) add nothing beyond the path transactions. Complex hops (funding before the ordinal input, ordinal paid to `o1`+, etc.) MUST include those preceding input sources. Missing sats → the hop cannot be proven; omit provenance rather than guessing.
...
5. **Tip binding (parent remittance — settle-style)** — If step 4 fails, receivers **SHOULD** accept the package when all of the following hold (soft-latch / pre-broadcast remittance):
   ...
   * FIFO maps that `vin` onto `T`’s vout. Preceding input source sats MUST be known — fail closed if missing.
...
7. **Ancestry + FIFO** — For each consecutive pair `(child, parent)` in `path` with different transaction ids:
   * the child transaction in the BEEF MUST spend `parent` as some input `vin`;
   * that `vin` MUST be the FIFO ordinal input for the child vout (`sum(input sats before vin) === sum(output sats before vout)`, input satoshis === 1).
   If any preceding input’s satoshi value is unknown (source tx missing from the BEEF and not otherwise known), verification MUST fail. For parent-remittance acceptance (step 5), the spend + FIFO check `T → P` is applied to transaction `T` (which MAY be obtained outside `beefB64`, but preceding input sources still MUST be available).
...
* This BRC does not alter BRC-62/95 encodings; it only constrains how they are used inside `customInstructions.provenance`. BEEF/AtomicBEEF prove SPV ancestry, not ordinal FIFO by themselves.
...
* Partial BEEF (structurally valid but missing a path transaction or a preceding input source needed for FIFO) MUST fail verification.
* [BRC-156](./0156.md) defines an optional **latched** profile (v3 provenance, O(1) verify) for wallets that adopt Commit/Settle transfers; v2 remains valid for legacy tips.
...
* **FIFO vs spend-only** — Spending a 1-sat parent does not prove that output received *that* sat. Verifiers that skip FIFO can be convinced an unrelated 1-sat output (e.g. a funding slice paid as 1 sat before the ordinal lands) carries the origin.
...
2. [BRC-156](./0156.md) — Latched 1Sat Provenance for Basket `1sat` (draft)

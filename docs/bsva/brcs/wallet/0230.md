...
Optional `live: true` queries the overlay directly (SLAP discovery + multi-host
failover) without reading the local cache. Response includes `live`, `host`, and
`hostsTried` metadata.

#### 4.6 `overlayLookup`

Live BRC-24 lookup — authoritative overlay state, **no local persistence**:
...
When `packId` is set, the wallet merges discovery hints from the installed manifest.
Wallets SHOULD resolve hosts in this order:

1. Manifest `overlayBaseUrl` and `discovery.hosts` (curator hints)
2. SLAP (`ls_slap`) on BSVA trackers ([BRC-88](./../overlays/0088.md))
3. GET `/listLookupServiceProviders` verification per candidate host
4. POST `/lookup` with failover until one host succeeds
...
```json
{
  ...
  "hostsTried": ["https://market.handcash.io", "https://overlay.example"],
  "truncated": false
}
```
...
1. Resolve overlay hosts:
   - Curator hints: manifest `overlayBaseUrl`, optional `discovery.hosts`
   - SLAP discovery: POST `ls_slap` on configured trackers with `{ "service": "<lookupService>" }`
   - Verify each candidate via GET `/listLookupServiceProviders`
2. POST BRC-24 `/lookup` on the first verified host; on failure, try the next host.

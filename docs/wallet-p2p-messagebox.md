...
| Pay-into-messagebox (BRC-29 remittance) | **Used for HandCash peers** — tip / pay-sent / Send-to-friend | Sender signs (`noSend`) and delivers Atomic BEEF + remittance via messagebox; **payee broadcasts**. Sender broadcasts only if delivery fails (box ack without BEEF `/files` is not delivery). |
| Soft-latch item send | Sender `postBeef` then tip-hint txid | Same P2P settle as BRC-29: `ItemSettlePath` + `noSend` Atomic BEEF to peer; payee internalizes + broadcasts. Sender broadcast only after `DELIVER_FAILED` (including BEEF upload failure) or for self/external address. |

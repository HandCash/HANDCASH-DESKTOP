...
| Pay-into-messagebox (BRC-29 remittance) | **Used for HandCash peers** — tip / pay-sent / Send-to-friend | `brc29SendMachine`: remittance ± inline `beefB64` on `sendMessage` (not `/files`). After inbox delivery, sender silently `postBeef`. Inbox unreachable → `brc29:` claim receipt (QR / copy). Inbox not ACKed until ingest. |
